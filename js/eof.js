...
valid_opcodes = addRange(valid_opcodes, [0xb0, 0xb2 + 1]);
...
const terminating_opcodes = [ 0x00, 0xf3, 0xfd, 0xfe, 0xff, 0xb1 ];
...
function validate_eof(code) {
  ...
  while (true) {
    ...
    if (section_id == S_TYPE && section_sizes[S_CODE] != 0) {
      throw "code section preceding type section";
    }
    ...
    if (section_id == S_CODE) {
      var total_code_sections = parseInt(byte_at(code, pos) + byte_at(code, pos+1), 16);
      pos += 2;
      for (var i=0; i < total_code_sections; i++) {
        if ((pos + 1) >= code_len) {
          throw "truncated code section size";
        }
        var section_size = parseInt(byte_at(code, pos) + byte_at(code, pos+1), 16);
        section_sizes[S_CODE].push(section_size);
        pos += 2;
      }
    } else {
      var section_size = parseInt(byte_at(code, pos) + byte_at(code, pos+1), 16);
    ...
    // Empty section different than data
    if (section_size == 0 && section_id != S_DATA) {
      throw "empty section";
    }
  }
  ...
  if (section_sizes[S_TYPE].length == 0) {
    throw "type section not found";
  }

  // Type section has size corresponding to number of code sections
  if (section_sizes[S_TYPE][0] != (section_sizes[S_CODE].length * 4)) {
  ...
  if (code_len != sections_size) {
    console.log(code_len, sections_size);
